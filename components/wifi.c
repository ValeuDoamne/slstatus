...
#define RSSI_TO_PERC(rssi) \
	...
	const char *
	wifi_print_preatty(const char *interface)
	{
		static char essid[IW_ESSID_MAX_SIZE+1];
		static char power[4] = { 0 };
		uint32_t pwr = 0;
		if(wifi_essid(interface) == NULL || wifi_perc(interface) == NULL)
			return bprintf("");
		strncpy(essid, wifi_essid(interface), IW_ESSID_MAX_SIZE);
		strncpy(power, wifi_perc(interface), 3);
